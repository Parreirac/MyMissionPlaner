...
namespace MilitaryPlanner.ViewModels
{
    public class MissionViewModel : BaseViewModel
    {
        ...
        public Mission CurrentMission
        {
            ...
            set
            {
                _mission = value; // TODO PRA : peut on eviter si _mission == value ?
                _phaseSymbols.Clear();
                ProcessMission();
        ...
        public MissionViewModel()
        {
            ...
            // SymbolLoader.MilitarySymbolDictionary  =  new DictionarySymbolStyle/*SymbolDictionary*/(SymbolDictionaryType.Mil2525c);
            //   SymbolLoader MilitarySymbolDictionary = SymbolLoader.Get();// new DictionarySymbolStyle/*SymbolDictionary*/(SymbolDictionaryType.Mil2525c);
            ...
            Mediator.Register(Constants.ACTION_MISSION_LOADED, OnMissionCloned); // Modif pra 
        ...
        public TimeExtent MissionTimeExtent
        {
            get
            {
                return new TimeExtent(_mission.PhaseList.Min(t => t.VisibleTimeExtent.StartTime), _mission.PhaseList.Max(t => t.VisibleTimeExtent.EndTime));
            }
        }
        ...
        public MissionPhase CurrentPhase
        {
            ...
            set
            {
                _currentPhase = value;// TODO PRA tester si on n'en fait pas trop 
                RaisePropertyChanged(() => CurrentPhase);
            }
        }
        ...
        private void ProcessMission()
        {
            ...
            int currentStartPhase;
            int currentEndPhase;
            var mes = _mission.GetMessages();
            foreach (var m in mes)
            {
                var mm = m as TimeAwareMilitaryMessage;
                currentStartPhase = 0;
                currentEndPhase = -1;

                if (mm != null)
                {
                    foreach (var phase in _mission.PhaseList) // TODO PRA pourquoi regarder toutes les phases ?
                    {

                        if (mm.VisibleTimeExtent.EndTime > phase.VisibleTimeExtent.StartTime && mm.VisibleTimeExtent.StartTime < phase.VisibleTimeExtent.EndTime)//mm.VisibleTimeExtent.Intersects(phase.VisibleTimeExtent))
                        {
                    ...
                    {
                        ID = mm.Id,
                        VisibleTimeExtent = new List<PropertyItem>() { new PropertyItem { Key = "StartTime", Value = mm.VisibleTimeExtent.StartTime.ToString() }, new PropertyItem { Key = "EndTime", Value = mm.VisibleTimeExtent.EndTime.ToString() } }
                    };
                    ...
                    CreateUpdateSymbolWithPM(m, currentStartPhase, currentEndPhase);
            }
        ...
        private void CreateUpdateSymbolWithPM(PersistentMessage pm, int currentStartPhase, int currentEndPhase)
        {
            ...
            // L'id utilisé dans cette liste est l'id du message, pas le code symbol.
            var foundSymbol = _phaseSymbols.FirstOrDefault(sl => sl.Id == pm.ID);// sl => sl.ItemSVM.Model.Values.ContainsKey(Message.IdPropertyName) && sl.ItemSVM.Model.Values[Message.IdPropertyName] == pm.ID);
            ...
            if (foundSymbol != null)
            {
                ...
                if (ps.EndPhase < currentEndPhase) // Modif pra ajout d'un test
                    ps.EndPhase = currentEndPhase;
            }
            else
            {
                // symbol is missing, ADD a new one
                PropertyItem first = pm.PropertyItems.FirstOrDefault(pi => pi.Key == Message.SicCodePropertyName);
                    ...
                    /*     var psvm = new PhaseSymbolViewModel
                         {
                             ...
                             ItemSVM = SymbolLoader.SearchAsync(first.Value),
                             VisibleTimeExtent = pm.VisibleTimeExtent
                         }; */

                    IEnumerable<SymbolProperties> value = DataLoader.SearchBySIC(first.Value, DataLoader.SearchMode.Large);
                    //      var t = new TimeExtent(DateTimeOffset.Parse(pm.VisibleTimeExtent.First(g => g.Key == "StartTime").Value),
                    //         DateTimeOffset.Parse(pm.VisibleTimeExtent.First(g => g.Key == "EndTime").Value));

                    //     var ga = value.FirstOrDefault().GraphicObject as Symbol;
                    var psvm = new PhaseSymbolViewModel
                    {
                        Id = pm.ID,
                        StartPhase = currentStartPhase,
                        EndPhase = currentEndPhase,
                        ItemSVM = new SymbolViewModel(value.FirstOrDefault(), Constants._imageSize),//new SymbolViewModel(ga.Clone(), Constants._imageSize),// ;// value.FirstOrDefault(),
                        VisibleTimeExtent = new TimeExtent(DateTimeOffset.Parse(pm.VisibleTimeExtent.First(g => g.Key == "StartTime").Value),
                        DateTimeOffset.Parse(pm.VisibleTimeExtent.First(g => g.Key == "EndTime").Value))
                    };

                    // create SVM  // REM : pra On ne doit pas stoker la clé dans values du model qui peut déjà avoir cette clé.
        ...
        private void CreateUpdateSymbolWithPM(SimpleMessage pm, int currentStartPhase, int currentEndPhase)
        {

            if (pm.IsTemporary && pm.Layer != null)
                return;
            // is this an update or a new symbol
            // L'id utilisé dans cette liste est l'id du message, pas le code symbol.
            var foundSymbol = _phaseSymbols.FirstOrDefault(sl => sl.Id == pm.Id);// sl => sl.ItemSVM.Model.Values.ContainsKey(Message.IdPropertyName) && sl.ItemSVM.Model.Values[Message.IdPropertyName] == pm.ID);
            ...
            {
                // symbol is missing, ADD a new one
                var first = pm.symbolProperties.SymbolID; // [Message.SicCodePropertyName];// ..getProperty();
                ...
                if (first != null)
                {
                    ...
                    IEnumerable<SymbolProperties> value = DataLoader.SearchBySIC(first, DataLoader.SearchMode.Large);
                    //      var t = new TimeExtent(DateTimeOffset.Parse(pm.VisibleTimeExtent.First(g => g.Key == "StartTime").Value),
                    //         DateTimeOffset.Parse(pm.VisibleTimeExtent.First(g => g.Key == "EndTime").Value));
                    if (!value.Any())
                    {
                        DataLoader.CreateNewSymbol(first);
                         value = DataLoader.SearchBySIC(first, DataLoader.SearchMode.Strict);
                    }
                    //     var ga = value.FirstOrDefault().GraphicObject as Symbol;
                    var psvm = new PhaseSymbolViewModel
                    {
    ...
    public class PhaseSymbolViewModel : BaseViewModel
    {
        ...
        public string Id { get; set; } // Ajout pra, plutot que d'aller sur model, on stocke ici
        ...
        private TimeExtent _visibleTimeExtent = new TimeExtent(DateTimeOffset.MaxValue);// modif pra plus de constructeur sans parametres
    ...
    public class PadLeftVariableWidthConverter : IMultiValueConverter
    {
        ...
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            // modif pra start => StartTime
            // end => endTime
            TimeSpan ts = messageTimeExtent.StartTime.Subtract(missionTimeExtent.StartTime);//.Start);
            TimeSpan mts = missionTimeExtent.EndTime.Subtract(missionTimeExtent.StartTime);// Start);

            var widthFactor = ts.TotalSeconds / (mts.TotalSeconds + 1);
    ...
    public class VariableWidthConverter : IMultiValueConverter
    {
        ...
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            TimeSpan ts = messageTimeExtent.EndTime.Subtract(messageTimeExtent.StartTime);
            TimeSpan mts = missionTimeExtent.EndTime.Subtract(missionTimeExtent.StartTime);
    ...
    public class SIC2BrushConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            Brush brush = sic[1].ToString().ToLower() switch
            {
                "f" => Brushes.DeepSkyBlue,
                "n" => Brushes.LightGreen,
                "h" => Brushes.Salmon,
                _ => Brushes.Yellow,
            };
            return brush;
