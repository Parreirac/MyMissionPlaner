    ...
    public class FrameworkElementDragBehavior : Behavior<FrameworkElement>
    {
        ...
        void AssociatedObject_MouseLeave(object sender, MouseEventArgs e)
        {
            if (_isMouseClicked)
            {
                ...
                if (dragObject != null && dragObject.IsDragable && !dragObject.HasBeenDragged)  // TODO PRA pourquoi dragObject.HasBeenDragged ? 
                {
                    DataObject data = new DataObject();

                    data.SetData(dragObject.DataType, AssociatedObject.DataContext);
                    data.SetData(typeof(FrameworkElement), sender as FrameworkElement);
