...
<iso:schema queryBinding="xslt2" xmlns:iso="http://purl.oclc.org/dsdl/schematron">
  ...
  <iso:ns uri="urn:fra:nc1:message:medsitrep" prefix="nc1medsitrep" />
  <iso:pattern>
    <iso:title>NC1_MedSitRep validation rules</iso:title>
    <iso:rule context="/nc1medsitrep:NC1_MedSitRep/medicalFacilityReport/facility">
      <iso:assert test="count(tacticalData) + count(medicalFacilityData) + count(location) = 3" diagnostics="D26544-en">NC1_MedSitRep.2: Les données tactiques, les propriétés spécifiques à l'installation médicale et la localisation de l'installation doivent être renseignées.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1medsitrep:NC1_MedSitRep/medicalFacilityReport/facility/tacticalData">
    ...
    <iso:rule context="/nc1medsitrep:NC1_MedSitRep/medicalFacilityReport/facility/tacticalData/symbolCode">
    ...
    <iso:rule context="/nc1medsitrep:NC1_MedSitRep/medicalFacilityReport/facility/@id">
      <iso:assert test="matches(.,'^18:')" diagnostics="D26363-en">NC1_Facility.5: Le type de l'objet doit avoir pour valeur 18</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1medsitrep:NC1_MedSitRep/medicalFacilityReport/facility/tacticalData/responsibleUnitId/@id">
    ...
    <iso:rule context="/nc1medsitrep:NC1_MedSitRep/medicalFacilityReport/facility/medicalFacilityData/admittedIndividualId/@id">
  ...
  <iso:diagnostics>
    ...
    <iso:diagnostic xml:lang="en" id="D26544-en">NC1_MedSitRep.2: The tactical data, the properties specific to the medical installation and the location of the installation must be informed.</iso:diagnostic>
  </iso:diagnostics>
</iso:schema>