...
<iso:schema queryBinding="xslt2" xmlns:iso="http://purl.oclc.org/dsdl/schematron">
  ...
  <iso:ns uri="urn:fra:nc1:message:movorder" prefix="nc1movorder" />
  <iso:pattern>
    <iso:title>NC1_MovOrder validation rules</iso:title>
    <iso:rule context="/nc1movorder:NC1_MovOrder//mixedText/anyReference/@type">
      <iso:assert test=".=('refObject','localisedPoint','fileLink','datetime')" diagnostics="D26472-en">NC1_MovOrder.2: L’attribut « type » est restreint aux valeurs suivantes : 'refObject', 'localisedPoint', 'datetime' et 'fileLink'.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1movorder:NC1_MovOrder//mixedText/anyReference[@type='refObject']">
      <iso:assert test="count(@id)=1 and count(@datetime)=0 and count(@x)=0 and count(@y)=0 and count(@z)=0 and count(@attachedFile)=0" diagnostics="D26473-en">NC1_MovOrder.3: Pour une référence à un objet, seuls les attributs « type », « id » et « instance » peuvent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1movorder:NC1_MovOrder//mixedText/anyReference[@type='datetime']">
      <iso:assert test="count(@id)=0 and count(@instance)=0 and count(@datetime)=1 and count(@x)=0 and count(@y)=0 and count(@z)=0 and count(@attachedFile)=0" diagnostics="D26474-en">NC1_MovOrder.4: Pour un groupe date heure, seuls les attributs « type » et « datetime » doivent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1movorder:NC1_MovOrder//mixedText/anyReference[@type='localisedPoint']">
      <iso:assert test="count(@id)=0 and count(@instance)=0 and count(@datetime)=0 and count(@x)=1 and count(@y)=1 and count(@attachedFile)=0" diagnostics="D26475-en">NC1_MovOrder.5: Pour un point localisé, seuls les attributs « type », « x », « y » et « z » peuvent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1movorder:NC1_MovOrder//mixedText/anyReference[@type='fileLink']">
      <iso:assert test="count(@id)=0 and count(@instance)=0 and count(@datetime)=0 and count(@x)=0 and count(@y)=0 and count(@z)=0 and count(@attachedFile)=1" diagnostics="D26476-en">NC1_MovOrder.6: Pour un lien vers un fichier, seuls les attributs « type » et « attachedFile » doivent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1movorder:NC1_MovOrder//mixedText">
      <iso:assert test="matches(.,'^[A-Za-z0-9àâäéèêëïîôöùûüÿç .,\-\(\)?!@#$%^&amp;*=_+\[\]{}\\&quot;'';&lt;&gt;~|€/:\n\rÇåìÄÅÉæÆòÖÜøØáíóúñÑ¡«»ÁÂÀãÃðÐÊËÈÍÎÏÌÓßÔÒõÕµÚÛÙýÝ§°¿¢®&#x0009;&#x0060;&#x00A0;&#x0152;&#x0153;&#x2012;&#x2013;&#x2019;&#x2026;]*$')" diagnostics="D26477-en">NC1_MovOrder.7: L'un des caractères utilisés n'est pas dans la liste des caractères autorisés.</iso:assert>
    </iso:rule>
  ...
  <iso:diagnostics>
    ...
    <iso:diagnostic xml:lang="en" id="D26472-en">NC1_MovOrder.2: The attribute « type » is restricted to the following values: 'refObject', 'localisedPoint', 'datetime' and  'fileLink'.</iso:diagnostic>
    <iso:diagnostic xml:lang="en" id="D26473-en">NC1_MovOrder.3: For a reference to an object, only the « type », « id » and «  context »  attributes can be informed.</iso:diagnostic>
    <iso:diagnostic xml:lang="en" id="D26474-en">NC1_MovOrder.4: For a group date the hour, only the « type » and « datetime »  attributes must be informed.</iso:diagnostic>
    <iso:diagnostic xml:lang="en" id="D26475-en">NC1_MovOrder.5: For a localized point, only the « type », « x », « y » and « z » attributes can be informed.</iso:diagnostic>
    <iso:diagnostic xml:lang="en" id="D26476-en">NC1_MovOrder.6: For a link towards a file, only the ""type"" and ""url""attributes  must be informed.</iso:diagnostic>
    <iso:diagnostic xml:lang="en" id="D26477-en">NC1_MovOrder.7: One character is not allowed.</iso:diagnostic>
