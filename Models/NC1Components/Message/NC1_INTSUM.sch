...
<iso:schema queryBinding="xslt2" xmlns:iso="http://purl.oclc.org/dsdl/schematron">
  ...
  <iso:ns uri="urn:fra:nc1:message:intsum" prefix="nc1intsum" />
  <iso:pattern>
    <iso:title>NC1_INTSUM validation rules</iso:title>
    <iso:rule context="/nc1intsum:NC1_INTSUM//mixedText/anyReference/@type">
      <iso:assert test=".=('refObject','localisedPoint','fileLink','datetime')">NC1_INTSUM.2: L’attribut « type » est restreint aux valeurs suivantes : 'refObject', 'localisedPoint', 'datetime' et 'fileLink'.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1intsum:NC1_INTSUM//mixedText/anyReference[@type='refObject']">
      <iso:assert test="count(@id)=1 and count(@datetime)=0 and count(@x)=0 and count(@y)=0 and count(@z)=0 and count(@attachedFile)=0">NC1_INTSUM.3: Pour une référence à un objet, seuls les attributs « type », « id » et « instance » peuvent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1intsum:NC1_INTSUM//mixedText/anyReference[@type='datetime']">
      <iso:assert test="count(@id)=0 and count(@instance)=0 and count(@datetime)=1 and count(@x)=0 and count(@y)=0 and count(@z)=0 and count(@attachedFile)=0">NC1_INTSUM.4: Pour un groupe date heure, seuls les attributs « type » et « datetime » doivent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1intsum:NC1_INTSUM//mixedText/anyReference[@type='localisedPoint']">
      <iso:assert test="count(@id)=0 and count(@instance)=0 and count(@datetime)=0 and count(@x)=1 and count(@y)=1 and count(@attachedFile)=0">NC1_INTSUM.5: Pour un point localisé, seuls les attributs « type », « x », « y » et « z » peuvent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1intsum:NC1_INTSUM//mixedText/anyReference[@type='fileLink']">
      <iso:assert test="count(@id)=0 and count(@instance)=0 and count(@datetime)=0 and count(@x)=0 and count(@y)=0 and count(@z)=0 and count(@attachedFile)=1">NC1_INTSUM.6: Pour un lien vers un fichier, seuls les attributs « type » et « attachedFile » doivent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1intsum:NC1_INTSUM//mixedText">
      <iso:assert test="matches(.,'^[A-Za-z0-9àâäéèêëïîôöùûüÿç .,\-\(\)?!@#$%^&amp;*=_+\[\]{}\\&quot;'';&lt;&gt;~|€/:\n\rÇåìÄÅÉæÆòÖÜøØáíóúñÑ¡«»ÁÂÀãÃðÐÊËÈÍÎÏÌÓßÔÒõÕµÚÛÙýÝ§°¿¢®&#x0009;&#x0060;&#x00A0;&#x0152;&#x0153;&#x2012;&#x2013;&#x2019;&#x2026;]*$')">NC1_INTSUM.7: L'un des caractères utilisés n'est pas dans la liste des caractères autorisés.</iso:assert>
    </iso:rule>
