...
<iso:schema queryBinding="xslt2" xmlns:iso="http://purl.oclc.org/dsdl/schematron">
  ...
  <iso:ns uri="urn:fra:nc1:message:cbrnsitrep" prefix="nc1cbrnsitrep" />
  <iso:pattern>
    <iso:title>NC1_CBRN-SITREP validation rules</iso:title>
    <iso:rule context="/nc1cbrnsitrep:NC1_CBRN-SITREP//mixedText/anyReference/@type">
      <iso:assert test=".=('refObject','localisedPoint','fileLink','datetime')" diagnostics="D26670-en">NC1_CBRN-SITREP.1: L’attribut « type » est restreint aux valeurs suivantes : 'refObject', 'localisedPoint', 'datetime' et 'fileLink'.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1cbrnsitrep:NC1_CBRN-SITREP//mixedText/anyReference[@type='refObject']">
      <iso:assert test="count(@id)=1 and count(@datetime)=0 and count(@x)=0 and count(@y)=0 and count(@z)=0 and count(@attachedFile)=0" diagnostics="D26671-en">NC1_CBRN-SITREP.2: Pour une référence à un objet, seuls les attributs « type », « id » et « instance » peuvent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1cbrnsitrep:NC1_CBRN-SITREP//mixedText/anyReference[@type='datetime']">
      <iso:assert test="count(@id)=0 and count(@instance)=0 and count(@datetime)=1 and count(@x)=0 and count(@y)=0 and count(@z)=0 and count(@attachedFile)=0" diagnostics="D26672-en">NC1_CBRN-SITREP.3: Pour un groupe date heure, seuls les attributs « type » et « datetime » doivent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1cbrnsitrep:NC1_CBRN-SITREP//mixedText/anyReference[@type='localisedPoint']">
      <iso:assert test="count(@id)=0 and count(@instance)=0 and count(@datetime)=0 and count(@x)=1 and count(@y)=1 and count(@attachedFile)=0" diagnostics="D26673-en">NC1_CBRN-SITREP.4: Pour un point localisé, seuls les attributs « type », « x », « y » et « z » peuvent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1cbrnsitrep:NC1_CBRN-SITREP//mixedText/anyReference[@type='fileLink']">
      <iso:assert test="count(@id)=0 and count(@instance)=0 and count(@datetime)=0 and count(@x)=0 and count(@y)=0 and count(@z)=0 and count(@attachedFile)=1" diagnostics="D26674-en">NC1_CBRN-SITREP.5: Pour un lien vers un fichier, seuls les attributs « type » et « attachedFile » doivent être renseignés.</iso:assert>
    </iso:rule>
    <iso:rule context="/nc1cbrnsitrep:NC1_CBRN-SITREP//mixedText">
      <iso:assert test="matches(.,'^[A-Za-z0-9àâäéèêëïîôöùûüÿç .,\-\(\)?!@#$%^&amp;*=_+\[\]{}\\&quot;'';&lt;&gt;~|€/:\n\rÇåìÄÅÉæÆòÖÜøØáíóúñÑ¡«»ÁÂÀãÃðÐÊËÈÍÎÏÌÓßÔÒõÕµÚÛÙýÝ§°¿¢®&#x0009;&#x0060;&#x00A0;&#x0152;&#x0153;&#x2012;&#x2013;&#x2019;&#x2026;]*$')" diagnostics="D26675-en">NC1_CBRN-SITREP.6: L'un des caractères utilisés n'est pas dans la liste des caractères autorisés.</iso:assert>
    </iso:rule>
  ...
  <iso:diagnostics>
    ...
    <iso:diagnostic xml:lang="en" id="D26670-en">NC1_CBRN-SITREP.1: The attribute « type » is restricted to the following values: 'refObject', 'localisedPoint', 'datetime' and  'fileLink'.</iso:diagnostic>
    <iso:diagnostic xml:lang="en" id="D26671-en">NC1_CBRN-SITREP.2: For a reference to an object, only the « type », « id » and «  context »  attributes can be informed.</iso:diagnostic>
    <iso:diagnostic xml:lang="en" id="D26672-en">NC1_CBRN-SITREP.3: For a group date the hour, only the « type » and « datetime »  attributes must be informed.</iso:diagnostic>
    <iso:diagnostic xml:lang="en" id="D26673-en">NC1_CBRN-SITREP.4: For a localized point, only the « type », « x », « y » and « z » attributes can be informed.</iso:diagnostic>
    <iso:diagnostic xml:lang="en" id="D26674-en">NC1_CBRN-SITREP.5: For a link towards a file, only the ""type"" and ""url""attributes  must be informed.</iso:diagnostic>
    <iso:diagnostic xml:lang="en" id="D26675-en">NC1_CBRN-SITREP.6: One character is not allowed.</iso:diagnostic>
